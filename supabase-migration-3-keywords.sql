-- =========================================================
-- Migration 3: add keywords to each resource, editable in God Mode
-- Run this in: Supabase Dashboard > SQL Editor > New query
-- (Run this AFTER migrations 1 and 2 have already been run)
-- =========================================================

-- 1. New column: a plain list of extra search terms per resource
alter table public.resources add column if not exists keywords text[] not null default '{}';
...
drop function if exists public.gm_add_resource(text, text, text, text, text, text, text, text, boolean, boolean);
drop function if exists public.gm_update_resource(text, uuid, text, text, text, text, text, text, text, boolean, boolean);

-- 3. Recreate both with the new p_keywords parameter
...
begin
  ...
  insert into public.resources
    (name, description, url, category, icon, tool_username, tool_password, is_internal_only, is_client_safe, keywords)
  values
    (p_name, p_description, p_url, p_category, p_icon, p_tool_username, p_tool_password, p_is_internal_only, p_is_client_safe, p_keywords)
